...
static void OnTick()
{
	for (Object prop : GetAllProps())
	{
		...
		float Z = data.originalZ + (std::max(std::sin(((GET_GAME_TIMER() - data.startOffset) / 150.f) * MetaModifiers::m_fChaosMultiplier) * 2.5f * MetaModifiers::m_fChaosMultiplier, 0.f));
