...
static void OnStart()
{
	for (int i = 0; i < MetaModifiers::m_fChaosMultiplier; i++)
	{
		...
		for (Ped ped : GetAllPeds())
		{
			...
			{
				if (ARE_ANY_VEHICLE_SEATS_FREE(veh) && IS_VEHICLE_DRIVEABLE(veh, true) && GET_ENTITY_HEIGHT_ABOVE_GROUND(veh) >= 0)
				{
					vehs.push_back(veh);
				}
			}
			...
			for (int j = -1; j < targetVehMaxSeats - 1; j++)
			{
				if (IS_VEHICLE_SEAT_FREE(targetVeh, j, false))
				{
					SET_PED_INTO_VEHICLE(ped, targetVeh, j);
