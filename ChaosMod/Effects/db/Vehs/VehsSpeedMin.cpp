...
#define SPEED_THRESHOLD (1.f - (1.f / (MetaModifiers::m_fChaosMultiplier + 1.f))) // % of max speed must be reached
...
static void OnTick()
{
    ...
    if (!IS_PED_DEAD_OR_DYING(playerPed, false) && IS_PED_IN_ANY_VEHICLE(playerPed, false) && GET_IS_VEHICLE_ENGINE_RUNNING(veh))
    {
        ...
        {
            sprintf_s(charBuf, "Minimum: %s\nDetonation In: %.1fs", displayMinSpeed.c_str(), float(m_timeReserve) / 1000);
        }
        else if (m_timeReserve != WAIT_TIME && speedms > minSpeed)
        {
            sprintf_s(charBuf, "Minimum: %s\nDetonation In: %.1fs (Recovering)", displayMinSpeed.c_str(), float(m_timeReserve) / 1000);
        }
        else
        {
