...
static void OnTickX2()
{
	for (auto veh : GetAllVehs())
	{
		MODIFY_VEHICLE_TOP_SPEED(veh, 2.f * MetaModifiers::m_fChaosMultiplier);
		SET_VEHICLE_CHEAT_POWER_INCREASE(veh, 2.f * MetaModifiers::m_fChaosMultiplier);
	}
}
...
static void OnTickX10()
{
	for (auto veh : GetAllVehs())
	{
		MODIFY_VEHICLE_TOP_SPEED(veh, 10.f * MetaModifiers::m_fChaosMultiplier);
		SET_VEHICLE_CHEAT_POWER_INCREASE(veh, 10.f * MetaModifiers::m_fChaosMultiplier);
	}
}
...
static void OnTickX05()
{
	for (auto veh : GetAllVehs())
	{
		MODIFY_VEHICLE_TOP_SPEED(veh, .5f / MetaModifiers::m_fChaosMultiplier);
		SET_VEHICLE_CHEAT_POWER_INCREASE(veh, .5f / MetaModifiers::m_fChaosMultiplier);
	}
}
