...
static void OnTick()
{
	...
	if (IS_PED_SHOOTING(player)) 
	{
		...
		if (GET_CURRENT_PED_WEAPON(player, &weaponHash, 1) && GET_WEAPON_DAMAGE_TYPE(weaponHash) == 3)
		{
			...
			horizontalRecoil *= MetaModifiers::m_fChaosMultiplier;
			for (int i = 0; i < 10; i++)
			{
				SET_GAMEPLAY_CAM_RELATIVE_PITCH(GET_GAMEPLAY_CAM_RELATIVE_PITCH() + ((verticalRecoil * MetaModifiers::m_fChaosMultiplier) / 10), 1.f);
