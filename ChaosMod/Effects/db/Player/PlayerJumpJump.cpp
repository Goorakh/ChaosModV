...
static void OnTick()
{
	...
	if (lastCheck + (500 / MetaModifiers::m_fChaosMultiplier) < GET_GAME_TIMER() && !IS_PED_JUMPING(player))
	{
		if (IS_PED_IN_ANY_VEHICLE(player, false))
		{
			...
			{
				Memory::ApplyForceToEntityCenterOfMass(veh, 0, .0f, .0f, 200.f * MetaModifiers::m_fChaosMultiplier, true, false, true, true);
