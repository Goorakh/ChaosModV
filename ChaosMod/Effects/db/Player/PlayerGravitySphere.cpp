...
static void OnTick()
{
	...
	{
		lastTick = curTick;
		constSpeed = Vector3(g_Random.GetRandomFloat(-2.f, 2.f), g_Random.GetRandomFloat(-2.f, 2.f), g_Random.GetRandomFloat(-0.3f, 2.f)); // make entities move above the player
	}
	...
	for (Entity entity : entities)
	{
		...
		if (distance < startDistance)
		{
			...
			{
				SET_ENTITY_VELOCITY(entity, playerSpeed.x + constSpeed.x, playerSpeed.y + constSpeed.y, playerSpeed.z + constSpeed.z);
			}
		}
	}
}

static RegisterEffect registerEffect(EFFECT_PLAYER_GRAVITY_SPHERE, nullptr, nullptr, OnTick, EffectInfo
	{
		.Name = "Gravity Sphere",
