	...
	if (IS_CONTROL_PRESSED(0, 25))
	{
					...
					if (didHit)
					{
						if (IS_ENTITY_A_PED(entityHandle))
						{
							...
							*/
							...
							static const int boneIds[] = { 0x0, 0x2e28, 0xe39f, 0xf9bb, 0x3779, 0xca72, 0x9000, 0xcc4d, 0xe0fd, 0x5c01, 0x60f0, 0x60f1, 0x60f2, 0xfcd9, 0xb1c5, 0xeeeb, 0x49d9, 0x29d2, 0x9d4d, 0x6e5c, 0xdead, 0x9995, 0x796e };
							for (int id : boneIds)
							{
		...
		for (PedHitInfo info : selectedOffsets)
		{
			if (DOES_ENTITY_EXIST(info.pedHandle))
			{
				...
				float boxSize = 0.1;
				DRAW_BOX(destOffset.x - boxSize, destOffset.y - boxSize, destOffset.z - boxSize, destOffset.x + boxSize, destOffset.y + boxSize, destOffset.z + boxSize, 255, 0, 0, 100);
			}
		}
	}
	else
